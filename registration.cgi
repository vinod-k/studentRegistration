...
<div class="container">
  ...
  <ul class="errorMessages"></ul>
	...
	<div class="row">
      ...
      <div class="col-75">
        <input type="email" id="emailid" name="emailid" placeholder=" " required>
      </div>
    </div>
	<div class="row">
      <div class="col-25">
...
if ($input{"go"} eq "go") {
	my $query = "INSERT INTO student (Name,City,DOB,Street,State,Country,Pincode,ContactNumber,emailid,password) VALUES (\"$input{name}\",\"$input{city}\",\"$input{dob}\",\"$input{street}\",\"$input{state}\",\"$input{country}\",\"$input{pincode}\",\"$input{contactno}\",\"$input{emailid}\",\"$input{password}\")";
	print $query;
	my $insertQue = $dbh ->prepare($query);
	$insertQue->execute();
